...
#include "ui_zbringcontroldialog.h"
...
std::list<std::string> ZBringControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBringControlDialog::listMints;

ZBringControlDialog::ZBringControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBringControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBringControlDialog::~ZBringControlDialog()
{
    delete ui;
}

void ZBringControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBringControlDialog::updateList()
{
...
void ZBringControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBringControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBring_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBringControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBringControlDialog::GetSelectedMints()
{
...
void ZBringControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
