                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBRG ought to be enough for anybody." - Bill Gates, 2017
    ui->zBRGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBRGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBRG->setEnabled(false);
        ui->pushButtonMintzBRG->setToolTip(tr("zBRG is currently disabled due to maintenance."));

        ui->pushButtonSpendzBRG->setEnabled(false);
        ui->pushButtonSpendzBRG->setToolTip(tr("zBRG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBRGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBRG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBRG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBRG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBRG in ") +
...
void PrivacyDialog::on_pushButtonSpendzBRG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBRG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBRG
        sendzBRG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBRG
    sendzBRG();
}

void PrivacyDialog::on_pushButtonZBringControl_clicked()
{
    ZBringControlDialog* zBringControl = new ZBringControlDialog(this);
    zBringControl->setModel(walletModel);
    zBringControl->exec();
}

void PrivacyDialog::setZBringControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBringSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBRG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BringAddress"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBRGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBRG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BRG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBRG</b>";
    ...
    // use mints from zBring selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBringControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBringControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbring selector in case it was used
    ZBringControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBring Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bring, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBring Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBRG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBRG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBRG "));
    ui->labelzBRGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBRGSyncStatus->setVisible(fShow);
}
